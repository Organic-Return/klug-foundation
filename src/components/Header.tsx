                            ...
                            {column.title && (
                              column.titleUrl ? (
                                <Link
                                  ...
                                  onClick={() => setActiveDropdown(null)}
                                >
                                  <h3 className={`relative inline-block ${isSkiTown ? 'text-white' : 'text-gray-900'}`} style={{ fontSize: 'clamp(1.5rem, 3vw, 1.5rem)', fontWeight: 800, lineHeight: 1.3, letterSpacing: '-0.02em', fontFamily: 'var(--font-inter), system-ui, sans-serif' }}>
                                    {column.title}
                                ...
                                <h3 className={`mb-2 ${isSkiTown ? 'text-white' : 'text-gray-900'}`} style={{ fontSize: 'clamp(1.5rem, 3vw, 1.5rem)', fontWeight: 800, lineHeight: 1.3, letterSpacing: '-0.02em', fontFamily: 'var(--font-inter), system-ui, sans-serif' }}>
                                  {column.title}
                                </h3>
                              )
                            )}
                              ...
                              {column.links?.map((link, linkIndex) => (
                                <li key={linkIndex}>
                                  <Link
                                    ...
                                    onClick={() => setActiveDropdown(null)}
                                  >
                                    <span className={`relative inline-block text-xl font-bold capitalize ${isSkiTown ? 'text-white/90' : 'text-gray-700'}`}>
                                      {link.label}
                                      <span className="absolute bottom-0 left-0 w-full h-[2px] bg-[var(--color-gold)] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
                                    </span>
                                    {link.description && (
