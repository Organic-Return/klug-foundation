...
export default async function RootLayout({
  ...
  // Analytics IDs (Sanity > env var fallback)
  const gaId = settings?.apiKeys?.googleAnalyticsId || process.env.NEXT_PUBLIC_GOOGLE_ANALYTICS_ID;
  const gtmId = settings?.apiKeys?.googleTagManagerId || process.env.NEXT_PUBLIC_GOOGLE_TAG_MANAGER_ID;
  const fbPixelId = settings?.apiKeys?.facebookPixelId || process.env.NEXT_PUBLIC_FACEBOOK_PIXEL_ID;
  const gadsConversionId = settings?.apiKeys?.googleAdsConversionId || process.env.NEXT_PUBLIC_GOOGLE_ADS_CONVERSION_ID;
